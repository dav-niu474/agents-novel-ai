...
本仓库已是 **pnpm workspace monorepo**（`packages/core` + `apps/cli` + 将来的 `apps/web`）。在**仓库根目录**安装一次即可：

```bash
pnpm install                       # 安装整个 workspace
pnpm -r build                      # 拓扑顺序构建：先 @novel/core，再 cli
pnpm --filter @novel-studio/cli novel -- <subcommand>   # 跑 CLI
# 或：cd apps/cli && pnpm dev -- <subcommand>（tsx 直跑；需先 build 一次 @novel/core）
```

> CLI 的内核已抽到 `@novel/core`（运行时无关，Web 端复用同一份）。`apps/cli` 只保留 `bin / commands / workflows`（命令行展示层），所有 schema / 资产 IO / LLM / skill 编译都来自 `@novel/core`。
...
monorepo 两个包：`apps/cli`（命令行展示层）依赖 `packages/core`（`@novel/core`，运行时无关内核）。

```
packages/core/src/         # @novel/core — CLI 与（将来）Web 共用
├── schemas/               # Zod schemas
...
│   ├── world.ts / world-render.ts          # JSON canonical + MD projection 双写
│   ├── character.ts / character-render.ts   # 角色卡 / 索引 / 关系网
│   └── outline.ts / outline-render.ts       # Markdown-canonical + 9 字段完整度
...
├── commands/              # 每条子命令一个文件
└── workflows/             # 多步交互流程（@inquirer/prompts）
    ├── blueprint-flow.ts  # 10 步开书定盘
    ├── world-flow.ts      # 3 步建世界
    ├── character-flow.ts  # 捏角色
    ├── json-collect.ts    # 通用「LLM 起草 / 编辑器手填」结构化 JSON 收集器
    └── outline-flow.ts    # 3 步建大纲
```

> 边界规则：`core/` 不依赖 `commander` / `@inquirer/prompts`（已验证），所以可被任意运行时复用——Web Studio 的后端将 import 同一个 `@novel/core`（见 `../../docs/design/05-web-studio.md`）。
